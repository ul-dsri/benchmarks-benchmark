...
| --- | --- |
| `scripts/generate_table.py` | This script downloads the CSV files if they don't already exist locally. It accepts as arguments a sheet id, tab id, and desired output format (latex or markdown). This script is called by make while it is reading `table_list.csv`. |
